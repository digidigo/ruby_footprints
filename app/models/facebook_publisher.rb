...
class FacebookPublisher < Facebooker::Rails::Publisher  
  ...
  def new_footprint_feed_item_template
    ...
    full_story_template("{*actor*} stepped on {*target*}", "{*image_link*} <p>Don't let them get away with treating each other this way.</p>
     <p> {*call_to_action*}</p>")
  end
  ...
  def new_footprint_feed_item(footprint)
    ...
    data({:general_step_link => link_to("Step", footprints_url) ,
  ...
  def footprint_created(footprint)
    ...
    victim = Facebooker::User.new(footprint.victim.uid)
    attacker = Facebooker::User.new(footprint.attacker.uid)
    self.recipients(victim)  # Note this only works during a request cycle since the session is being held for us
    self.from(attacker)
  ...
  def nudge_story(user)
    ...
    if( db_friends.blank? )
      ...
      title = db_friends[0..3].compact.collect{|friend| fb_name(friend.uid)}.to_sentence + " have been stepping on people."
    end            
